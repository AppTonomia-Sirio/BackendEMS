...
class CustomUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = CustomUser
        fields = ('id', 'email', 'name', 'surname', 'password', 'is_superuser', 'created_at', 'is_staff')
...
class NNAUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = NNAUser
        fields = ('id', 'email', 'name', 'surname', 'password', 'created_at', 'document',
                  'date_of_birth', 'home', 'status', 'gender', 'mentors', 'therapist',
                  'autonomy_level', 'is_tutor', 'entered_at')
        extra_kwargs = {'password': {'write_only': True}, 'created_at': {'read_only': True}}
        def create(self, validated_data):
...
class StaffUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = StaffUser
        fields = ('id', 'email', 'name', 'surname', 'password', 'created_at',
                  'homes', 'roles', 'is_admin')
        extra_kwargs = {'password': {'write_only': True}, 'created_at': {'read_only': True}}
    def create(self, validated_data):
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomUser
        fields = ('id', 'email', 'name', 'surname', 'password', 'document', 'date_of_birth', 'home', 'roles', 'status', 'created_at',)
        extra_kwargs = {'password': {'write_only': True}, 'created_at': {'read_only': True}}
